...
├── /posts                          CREATE and manage posts: GeneratePostForm  ✅
│                                   (single/topic/bulk) above the filterable
│                                   grid, full lifecycle. Generation returned
│                                   here 2026-09-01 — it briefly lived in a
│                                   separate Content Creation module
...
COMPANY MANAGEMENT (a resolver shim, not a page — 2026-09-01)                ✅
/create → redirects to /companies/[active slug]
The nav item has to carry a static href (Navigation is a client component with
no session or cookie access), but the company it should open is only knowable
server-side. This route closes that gap: resolve the active company, redirect
into the workspace above. It renders NO company-selection UI of its own — the
header's selector is the app's only one — and no second copy of the workspace.
/create/[slug] is kept as a compatibility redirect to /companies/[slug] for
bookmarks and the company switcher. Its only rendered state is the empty
onboarding one, reached when the user has no accessible company at all.
...
| Decision                                                                                                                                           | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Global nav is Dashboard, Companies, Company Management, Competitive Analysis, Admin _(2026-09-01, supersedes both rows below)_                     | Two company entries answering two different questions: **Companies** is the list ("which companies exist / add one"), **Company Management** is the active one's workspace ("manage what I'm working in"). The 2026-08-31 position — that a company is only ever context, never a destination — proved too pure: managing a company IS a task, and it was the app's most-used one, reachable only through a header dropdown. The selector still owns _switching_; the nav owns _going there_. Because Company Management redirects into `/companies/[slug]`, active-state is a deliberate rule (`lib/navigation/nav-active.ts`), not a prefix match. |
| Post generation lives on the company's Posts tab, not a separate module _(2026-09-01, reverses the row below)_                                     | Generation and the posts it produces are one loop: generate → review the draft → approve → schedule. Splitting them put a route change in the middle of that loop and left the Posts tab with an empty state whose only action was "go somewhere else." The generator itself is unchanged — same `GeneratePostForm`, services, permissions, validation and scheduling; only the mount point moved back. What the split _did_ prove worth keeping is that generation must not own a top-level nav slot.                                                                                                                                               |
| ~~Global nav is Dashboard, Content Creation, Competitive Analysis, Admin — no sidebar, no "Companies" item~~ _(2026-08-31, superseded 2026-09-01)_ | A company is not a task, it is the context a task runs in. The two real global tasks are creating content and analyzing competitors; both need a company but are not "about" any one company the way the management workspace is. Representing the company as a persistent header selector, not a nav destination, keeps the nav answering "what am I doing" rather than "where am I."                                                                                                                                                                                                                                                               |
| User name, EN/BG switcher and Logout collapse into one avatar menu _(2026-09-01)_                                                                  | Three permanent controls, each rarely used, consumed roughly a third of the bar. One 32px avatar (profile image → initials → generic icon) opens a menu holding all three. No new mechanism: the same `NEXT_LOCALE` cookie write, now shared via `useLocaleSwitch`, and the same `logoutAction` server action.                                                                                                                                                                                                                                                                                                                                       |
